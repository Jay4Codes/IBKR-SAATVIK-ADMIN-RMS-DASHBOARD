...
def test_a_channel_is_recognised_however_the_bot_got_there():
    """Telegram delivers a channel's content as `channel_post`, never as
    `message`, and a promotion as `my_chat_member`. Listening only for messages
    meant a bot could sit in a channel as an administrator and hear nothing."""
...
def test_a_group_chat_is_recognised_so_its_id_can_be_read_off():
    """A private group has no username, so its id only ever arrives on an update."""
    assert telegram.group_chat(
    ...
    # A person's own chat is not a channel, and must never be treated as one.
...
def test_an_addressed_start_in_a_group_still_links():
    # Telegram rewrites the command in a group as "/start@thebot <code>".
...
async def test_named_move_levels_are_stored_alongside_the_repeating_band(client, stores):
    """A desk asking for 2%, 3% and 5% is naming levels, not a step size."""
    body = (await client.post("/api/v1/me/alerts", json={
        "triggers": ["move"], "move_levels": ["3", "2", "5"],
    })).json()["data"]
    assert body["move_levels"] == ["2", "3", "5"]
    # The band stays at its default, and is what a member without levels gets.
...
async def test_move_levels_are_magnitudes_and_deduplicated(client, stores):
    ...
    # Signs are dropped (a move is both ways), and nonsense with them.
